-- this file includes the 're' module from the 'lpeg' library modified to support unicode characters
...
local hex_prefix = m.P("0x") + m.P("0X")
local hex_digit = m.R("09", "af", "AF")
local hex_number = hex_prefix * hex_digit^1

local utf_char = "utf'" * m.C(hex_number) * "'"
...
local exp = m.P{ "Exp",
  ...
  Primary = "(" * m.V"Exp" * ")"
            ...
            + utf_char /
                function (n)
                    local utf_num = tonumber(n)
                    return mm.utfR(utf_num, utf_num)
                end
            + Class
            + defined
