...
local function assert_intervals(test_cases)
   for _, test_case in ipairs(test_cases) do
      ...
      luaunit.assertEquals(Interval.identify(note1, note2):semitones(), semitones,
         "Mismatched semitones for notes " .. tostring(note1) .. " " .. tostring(note2)
      )

      luaunit.assertEquals(Interval.identify(note1:toascii(), note2:toascii()):tostring(), interval,
         "Can't identify by strings" .. tostring(note1) .. " " .. tostring(note2)
      )

      luaunit.assertEquals(note1:transpose(Interval.fromstring(interval)), transposition,
         "Mismatched transposition result " .. tostring(note1) .. " " .. tostring(note2)
      )

      luaunit.assertEquals(note1:transpose(interval), transposition,
         "Can't tranpose by string" .. tostring(note1) .. " " .. tostring(note2)
      )

      luaunit.assertEquals(transposition:transpose_down(Interval.fromstring(interval)), note1,
         "Mismatched result for transpose_down " .. tostring(transposition) .. " " .. interval
      )

      luaunit.assertEquals(transposition:transpose_down(interval), note1,
...
function test_across_octaves()
   assert_intervals(across_octave)
end
...
function test_octave_unaware()
   assert_intervals(octave_unaware)
end
...
function test_unisons()
   assert_intervals(unison)
end
...
function test_interval_to_semitones()
   for interval, expected_semitones in pairs(interval_to_semitones) do
      luaunit.assertEquals(
         Interval.fromstring(interval):semitones(), expected_semitones,
